...
public class Timer extends Command {
	...
	public EmbedBuilder run(ExecutingCommand executingCommand) {
		...
		if (seconds > 0 && seconds <= 31536000) {
			...
			AlarmTask t = new AlarmTask(executingCommand.getChannel().getIdLong(), messageID, seconds, msg, executingCommand.getLanguage().getMsg(LanguageKey.TIMER_ALERT, new String[]{author.getAsMention()}));
			...
			return response(EmbedType.SUCCESS)
					.setDescription(executingCommand.getLanguage().getMsg(LanguageKey.TIMER_STARTED, new String[]{"**" + timeMsg + "**"}));
		} else {
			return response(EmbedType.ERROR, ErrorKey.TIMER_LENGTH_INCORRECT.name())
					.setDescription(executingCommand.getLanguage().getError(ErrorKey.TIMER_LENGTH_INCORRECT));
		}
	}
}